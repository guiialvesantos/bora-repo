-- 0017_sigma_anchor — σ ancorado na existência do SKU, atrás de flag.
--
-- REGRA CONCEITUAL: semana anterior à primeira venda do SKU não é demanda
-- zero. É ausência de observação.
--
-- O grid legado começa na primeira venda da EMPRESA: um SKU lançado há 10
-- semanas carrega ~48 semanas de zero pré-existência, e o salto 0→vendas
-- vira variância — σ inflado, ES superdimensionado exatamente nos
-- lançamentos (hoje, a coleção C50 que domina o pedido). Isto é CORREÇÃO DE
-- DEFINIÇÃO, não escolha de modelo: não há backtest que arbitre σ sem meses
-- de stockout_periods; o critério é provar que a regra atual contém
-- observações inválidas e medir o efeito por diff (feito antes do publish).
--
-- Desenho:
--   · O grid semanal continua o da empresa (mesmas fronteiras de 7 dias) —
--     só se excluem os buckets anteriores ao bucket da primeira venda do
--     SKU. Grade própria por SKU desalinharia as fases das semanas.
--   · Âncora = primeira venda, não "entrada em catálogo" (o Tiny não dá essa
--     data de forma confiável). Zeros reais iniciais são descartados junto —
--     σ levemente subestimado em vez de brutalmente superestimado.
--   · Proteções explícitas: sigma_weeks < 4 → low_confidence (já existia e
--     agora é verdadeiro em vez de decorativo); n < 2 → σ indefinido de
--     propósito (fallback documentado no sigma_calc, nada de valor
--     silencioso).
--   · Default 'global_history_legacy': aplicar esta migration não muda
--     nenhum número (checksum pré = pós). A troca é publish versionado.

do $$ begin
  create type public.sigma_anchor as enum ('global_history_legacy', 'sku_first_sale');
exception when duplicate_object then null; end $$;

alter table public.replenishment_params
  add column if not exists sigma_anchor public.sigma_anchor not null default 'global_history_legacy';

-- Só o corpo muda (buckets + sigma_calc); colunas de retorno idênticas às da
-- 0016, então `create or replace` preserva ACL — sem DROP, sem re-revoke.
