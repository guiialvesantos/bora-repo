-- 0032_product_analysis — a ficha de um produto só.
--
-- Uma consulta para a tela inteira, e não seis. A tela troca de produto num
-- seletor, então cada troca custa uma ida ao banco: seis idas por troca fariam
-- a tela piscar em pedaços, cada painel chegando na sua hora.
--
-- ---------------------------------------------------------------------------
-- O QUE NÃO ESTÁ AQUI, E POR QUÊ: histórico de estoque.
--
-- Não existe tabela de posição diária. E não dá para reconstruí-la com
-- fidelidade: `purchase_order_items.qty_received` é ZERO em toda a base e
-- `purchase_orders.eta_on` é nulo em todos os 310 pedidos — o ERP não datou
-- nenhum recebimento. Sem data de entrada, a única conta possível é
--
--     estoque(d-1) = estoque(d) + vendido(d)
--
-- que ignora reposição e portanto devolve um LIMITE SUPERIOR do que havia: a
-- peça pode ter entrado no meio do caminho, nunca saído sem venda. A conta é
-- feita no cliente, desenhada tracejada e chamada pelo nome. O que sai daqui é
-- só o que é verificável: venda por dia, que é exata.
--
-- A correção de verdade é passar a gravar a posição todo dia. Enquanto isso não
-- existe, dizer "limite superior" é mais honesto do que desenhar uma linha
-- cheia que se contradiz no primeiro reabastecimento.
-- ---------------------------------------------------------------------------
...
begin
  ...
  by_wh as (
    select coalesce(jsonb_agg(jsonb_build_object(
             'name', w.name, 'qty', ps.qty,
             'available', w.include_in_available
           ) order by ps.qty desc), '[]'::jsonb) as list
    from public.product_stock ps
    join public.warehouses w on w.id = ps.warehouse_id
    where ps.company_id = _company_id and ps.product_id = _product_id and ps.qty <> 0
  ),
  transit as (
  ...
  -- Venda do produto escolhido, dia a dia. Só os dias COM venda; a série
  -- completa (com os zeros) é montada no cliente, que já sabe a janela — mandar
  -- 180 zeros pela rede para o navegador redesenhar não paga a viagem.
  sales as (
  ...
  life as (
    ...
    from public.sales_order_items i
    join public.sales_orders o on o.id = i.order_id
    where i.company_id = _company_id and i.product_id = _product_id
      and not i.shadow and not o.shadow
  ),
  ...
  select jsonb_build_object(
    ...
    'product', jsonb_build_object(
      ...
      'image_url', prod.image_url, 'sale_price', prod.sale_price,
      'cmv', prod.cmv, 'category', prod.category, 'brand', prod.brand,
    ...
    'stock',      coalesce((select qty from stock where product_id = _product_id), 0),
  ...
  into _result
  from prod, by_wh, daily, windows, siblings, life;
