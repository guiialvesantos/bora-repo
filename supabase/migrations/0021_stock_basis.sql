-- 0021_stock_basis — estoque disponível para venda (saldo − reservado).
--
-- O Tiny devolve, por depósito, três números: `saldo` (físico), `reservado`
-- (peças comprometidas em pedidos de venda em aberto) e `disponivel`
-- (saldo − reservado). Até aqui o motor somava o saldo — peça já vendida e
-- não expedida contava como estoque, adiando compra que na prática já era
-- necessária.
--
-- Desenho:
--   · `product_stock.qty_reserved` guarda o reservado por depósito; o
--     disponível é derivado (qty − qty_reserved), nunca gravado — dois
--     números gravados poderiam discordar.
--   · A escolha é parâmetro do motor (`stock_basis`), não flag do depósito:
--     reserva é conceito da empresa inteira, e mudar a base do `E` muda
--     pedido de compra — tem que ser publish versionado, com snapshot antes
--     e depois, como toda chave do motor.
--   · Default 'saldo': aplicar esta migration não muda nenhum número
--     (harness de paridade intacto — as fixtures têm qty_reserved = 0).
--   · v2 (Triana) não escreve qty_reserved (fica 0) — saldo e disponível
--     coincidem até o conector v2 aprender reservas.

alter table public.product_stock
  add column if not exists qty_reserved numeric(14, 4) not null default 0;

do $$ begin
  alter table public.replenishment_params
    add column stock_basis text not null default 'saldo'
      constraint replenishment_params_stock_basis_check
      check (stock_basis in ('saldo', 'disponivel'));
exception when duplicate_column then null; end $$;

-- Só o `base` CTE muda (fonte do stock_total); colunas de retorno idênticas
-- às da 0017, então `create or replace` preserva ACL — sem DROP, sem re-revoke.
