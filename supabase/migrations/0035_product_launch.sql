-- 0035_product_launch — a curva de vida do produto, semana a semana desde a
-- primeira venda, e o que falta para o sell-through.
--
-- ---------------------------------------------------------------------------
-- POR QUE O SELL-THROUGH É EXATO AQUI, E A LINHA DE ESTOQUE NÃO É
--
-- A 0032 diz que não há histórico de recebimento: `qty_received` é zero na base
-- inteira. Isso condena a série de estoque a ser um limite superior. Mas NÃO
-- condena o sell-through, por causa da identidade de conservação:
--
--     estoque inicial + recebido = vendido + estoque final
--
-- Logo, para qualquer janela que termina hoje:
--
--     sell-through = vendido / (inicial + recebido) = vendido / (vendido + saldo de hoje)
--
-- O lado direito só usa venda (exata) e saldo de hoje (medido). O recebimento
-- que não existe se cancela. Por isso o sell-through entra na tela como número
-- cheio, e não tracejado como a linha de estoque.
--
-- A ressalva que sobra é outra: o saldo usado é o DISPONÍVEL
-- (`include_in_available`), então peça em Defeito ou Geral não entra no
-- denominador e empurra o número para cima. É a mesma definição do motor e da
-- cobertura — trocá-la aqui faria a ficha discordar do Pedido de compra.
-- Como é uma conta de cliente, ela não está neste arquivo; o que está aqui é o
-- que o cliente não consegue: a série semanal desde o lançamento.
--
-- ---------------------------------------------------------------------------
-- A SEMANA 1 NEM SEMPRE É O LANÇAMENTO
--
-- Não existe data de lançamento na base. `products.created_at` não serve: os
-- 2.079 produtos da All Out foram "criados" em DOIS dias — é a data do sync,
-- não a do produto. A única âncora possível é a primeira venda.
--
-- Só que a venda importada começa em 13/09/2025 (All Out) e 04/08/2025
-- (Triana). Quem já vendia antes disso aparece com primeira venda colada na
-- borda, e a "semana 1" dele é só o começo do DADO. Medido: 196 dos 1.348
-- produtos da All Out (14,5%) e 59 dos 261 da Triana (22,6%).
--
-- Por isso sai `launch_censored`: a tela precisa dizer "a curva começa onde o
-- dado começa" em vez de desenhar um lançamento que não aconteceu ali.
-- ---------------------------------------------------------------------------
...
begin
  ...
  into _result
  from prod, by_wh, daily, windows, launch, siblings, life, price;
