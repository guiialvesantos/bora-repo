...
>(({ side = 'right', className, children, hideClose = false, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
      {children}
      {!hideClose && (
        <SheetPrimitive.Close className="absolute right-4 top-4 rounded-full p-1 text-mono-500 transition-ui hover:bg-mono-200 hover:text-mono-1150 focus-visible:outline-none focus-visible:shadow-focus">
          <X className="h-4 w-4" />
